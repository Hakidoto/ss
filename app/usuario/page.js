...
export default function Page() {
    ...
    if (session) {
      try {
        ...
        const response = await fetch(
          `/api/usuario/experienciaLaboral/experiencia/${encodeURIComponent(
            user.RFC
          )}`
        );
        if (response.ok) {
  ...
  const renderTabContent = (item) => {
    switch (item.id) {
      ...
      case "workExperience":
        return <WorkExperience user = {user} userExp={userExp} isEditable = {isEditable} userId = {userId} fetchData= {fetchData} setIsEditable={setIsEditable} loading = {loading} userRfc = {userRfc}/>;
      default:
        return null;
    }
  };
  ...
  return (
    <Card className={`mx-auto my-auto ${style.main_card}`}>
      ...
      <CardBody>
              ...
              <Tab key={item.id} title={item.label}>
                <Card>
                  <CardBody className={style.bodyCard}>
                    {session? renderTabContent(item) : "no data"}
                  </CardBody>
                </Card>
              </Tab>
            )}
        ...
        <Button
          className={isEditable ? `w-1/2  ${style.hov}` : `w-1/2`}
          color="secondary"
          variant={isEditable ? "solid" : "flat"}
          size="md"
